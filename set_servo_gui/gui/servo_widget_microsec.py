...
class ServoControl(QWidget):
    ...
    def init_ui(self) -> None:
        ...
        for i in range(10, 16):
            ...
            microsec_label = QLabel(f"Microsec: {slider.value()}")
            self.microsec_labels.append(microsec_label)

            h_layout.addWidget(label)
            h_layout.addWidget(slider)
            h_layout.addWidget(microsec_label)
    ...
    def __update_servo(self, pwm_id: int, microsec: int) -> None:
        if self.device and self.device.is_connected():
            command_str = f"{pwm_id} {microsec}\n"
            self.device.send_data(command_str)
            print(f"Sent: {command_str.strip()}")

            self.microsec_labels[pwm_id-10].setText(f"Microsec: {microsec}")
